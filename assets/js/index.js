let iconos = ["fas fa-circle", "fas fa-circle", "fas fa-circle", "fas fa-circle", "fas fa-circle", "fas fa-circle", "fas fa-circle", "fas fa-circle", "fas fa-circle", "fas fa-circle"],
    info = ["A reference / link of the website where the design is will be requested. This reference must be of good quality, that way I will be able to appreciate the markings, colors, etc clearly. If the designs are made in a free third-party fonts (for example: Lioden), a screenshot must be sent with the categories of base colors and markings (also with the category of decorations if you want the decorations to be seen in the preset ).", "It must be indicated if a particular texture is desired for the preset, otherwise the textures that seem convenient to me will be used.", "I work with Photoshop, so if you want me to use files you already have to modify or make the preset, try to convert those files to PSD.", "Screenshots of the process will be sent as the preset is made, this is the moment to request changes. Refunds will not be accepted once the preset is finished, if I have made a mistake I will fix it for free.", "Once the preset has been delivered and the payment has been made, any additional or change that you want me to make must be paid.", "When you upload the .FHP file you must credit me for it.", "Once the full payment has been made, I will send you the preset files.", "If you have purchased a preset made by me from someone else, I will not make free edits.", "You agree not to make any changes to the preset without my permission.", "I will agree to make up to 5 presets. Each place on the waiting list will be occupied in the order of request and you will be notified of the waiting list, for that reason the preset may take time. DON'T rush me."],

//Commissions
    icon = ["fas fa-hashtag", "fas fa-image", "fas fa-paw", "fas fa-file-alt", "fas fa-plus"],
    titlecomm = ["Discord DM", "Reference", "Desired Texture", "Preset Number", "Extras"]
    infocomm = ["You can DM me by discord in case you want a commission.", "A reference will be necessary to make the preset.", "If you have one, you must tell me the texture you want me to use.", "The desired preset number must be specified. (optional)", "If you want to add an extra to the design of the preset, it must be communicated, as that will influence the price."],

//Payment
    imagesPaym = ["wu", "cpu", "ticket"],
    tituloPaym = ["Western Union", "Cash Pick Up", "Ticket"],
    infoPaym = ["I accept Western Union only (Preferably €, otherwise USD).", "You will choose the option 'Cash pick up' in the section 'How does your receiver want the money?'.", "You must send me a photo or screenshot of the ticket where the full name, the country from which it comes, the amount of money and the Tracking Number (MTCN) appear."],
...
function lista(){
    ...
    for(j = 0; j < 3; j++){
        document.getElementsByClassName("treecircles")[0].innerHTML += "<article><figure><img src='assets/images/" + imagesPaym[j] + ".png' class='image3'></figure><div class='payment'><h1>" + tituloPaym[j] + "</h1><p>" + infoPaym[j] + "</p></article>";
    }
...
function openLightbox(img){
    ...
    document.getElementById("title").innerHTML = "Image " + img + " of 3";
}
